...
namespace PuppyBotIMU {
    static PioEncoder encoder_PIO(27);
    static ICM20948 icm;
    ...
    int get_pulse_Encoder(){
        if(state_begin_Encoder == 0){
            state_begin_Encoder = 1;
            encoder_PIO.begin();
        }
        return encoder_PIO.getCount();
    }
    void resetEncoder(){
        encoder_PIO.reset();
    }
