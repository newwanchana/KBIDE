...
void DFRobot_VL53L0X::writeByteData(unsigned char Reg, unsigned char byte){
	Wire1.beginTransmission(_detailedData.i2cDevAddr); // transmit to device #8
	Wire1.write(Reg);              // sends one byte
	Wire1.write((uint8_t)byte);
...
void DFRobot_VL53L0X::readData(unsigned char Reg, unsigned char Num){
	...
	Wire1.beginTransmission(_detailedData.i2cDevAddr); // transmit to device #8
	Wire1.write((uint8_t)Reg);              // sends one byte
	Wire1.endTransmission();    // stop transmitting
    Wire1.requestFrom((uint8_t)_detailedData.i2cDevAddr, (uint8_t)Num);

	for(int i=0;i<Num;i++)
	{
		_detailedData.originalData[i] = Wire1.read();
		delay(1);
	}
}
...
uint8_t DFRobot_VL53L0X::readByteData(unsigned char Reg){
	...
	Wire1.write((uint8_t)Reg);              // sends one byte
	Wire1.endTransmission();    // stop transmitting
    Wire1.requestFrom((uint8_t)_detailedData.i2cDevAddr, (uint8_t)1);
	data = Wire1.read();
	return data;
}
