...
export default {
  ...
  data: {
    ...
    snackbar: function (text, color, x, y, timeout) {
      ...
      if (color) {
        this.snackbarConfig.color = color;
      }
      if (x) {
        this.snackbarConfig.x = x;
      }
      if (y) {
        this.snackbarConfig.y = y;
      }
      if (timeout) {
    ...
    addTab: function (tabName, tabTitle, tabComp, tabs) {
      ...
      setTimeout(() => {
        this.global().$emit("panel-resize", 100);
      }, 200);
    },
    ...
    reactiveTab: function () {
      ...
      if (!foundR && this.rightTab.length > 0) {
        this.rightTabModel =
          "rtab-" + this.rightTab[this.rightTab.length - 1].name;
      }
      if (!foundB && this.bottomTab.length > 0) {
        this.bottomTabModel =
          "btab-" + this.bottomTab[this.bottomTab.length - 1].name;
      }
    },
    /* tab helper function */
