...
const vueLoader = function (file) {
  ...
  if (validRegex.test(vue)) {
    let template = vue.replace(
      new RegExp("^[^]*<template>([^]+)</template>[^]*$", "gm"),
      "$1"
    );
    template = template.split("`").join("\\`");

    let script = vue.replace(
      new RegExp("^[^]*<script>([^]*)export default[^]+</script>[^]*$", "gm"),
      "$1"
    );
  ...
  filterFileName: function (obj, filterName) {
    ...
    Object.keys(obj).forEach((key) => {
      Object.keys(obj[key]).forEach((ckey) => {
        if (ckey.toLowerCase().startsWith(filterName.toLowerCase())) {
          //ignorcase naja
          if (!(key in res)) {
            res[key] = {};
          }
