  ...
  <div>
    <v-tooltip bottom>
      <v-btn
        ...
        slot="activator"
        icon
        @click="exampleDialog = !exampleDialog"
      >
    ...
    >
      <v-card>
        ...
        <smooth-scrollbar>
          <v-card-text>
            <v-subheader class="title"> Platform Examples </v-subheader>

            <div>
              <tree-menu
                :nodes="platformExample"
                v-on:open="openExample"
              ></tree-menu>
            </div>

            <v-divider></v-divider>

            <v-subheader class="title"> Board Examples </v-subheader>
            <div>
              <tree-menu
                :nodes="boardExample"
                v-on:open="openExample"
            ...
            <v-subheader class="title"> Plugin Examples </v-subheader>
            <div>
              <tree-menu2
                :nodes="pluginInfo"
                v-on:open="openExample"
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn class="btn-danger" flat @click.native="exampleDialog = false"
            >Close</v-btn
          >
        </v-card-actions>
      </v-card>
    ...
    window.addEventListener("keydown", (e) => {
      if (e.key === "Escape") {
        if (this.exampleDialog === true) {
          console.log(
            "---------> Do something when detect escape / Example & Tutorial"
          );
  ...
  methods: {
    createMenu: async function (root, dir) {
      ...
      try {
        ...
        if (
          examples.find(
            (el) =>
              el.endsWith(".md") || el.endsWith(".ino") || el.endsWith(".bly")
          )
        ) {
          return examples;
        } else {
    ...
    openExample(type, file) {
      ...
      win.loadURL(
        ...
        }?persistence=false&mode=${mode}&file=${file}`
      );
      win.show();
      //--tracking--//
