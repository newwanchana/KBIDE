<template>
  <v-tooltip bottom>
    <v-btn color="primary darken-2" slot="activator" icon @click="addPluginZip">
      <v-icon dark>fa-plug</v-icon>
    </v-btn>
    <span>Add Plugin.zip</span>
  </v-tooltip>
</template>
    ...
    addPluginZip() {
      ...
      ipcRenderer.once("open-zip-dialog-response", (event, res) => {
        ...
        if (
          ...
          const zipPath = res[0];
          console.log("📂 เริ่มติดตั้งปลั๊กอินจาก:", zipPath);
          ...
          try {
            ...
            const extractPath = path.join(appInstallPath, "plugins");
            ...
            this.$global.ui.snackbar(
              "✅ ติดตั้งปลั๊กอินสำเร็จ: " + path.basename(zipPath)
            );
          } catch (err) {
            console.error("❌ เกิดข้อผิดพลาดขณะติดตั้งปลั๊กอิน:", err);
            this.$global.ui.snackbar(
              "❌ ติดตั้งปลั๊กอินล้มเหลว โปรดตรวจสอบไฟล์ .zip"
            );
          }
        }
        // else {
        //   this.$global.ui.snackbar("❌ คุณยังไม่เลือกไฟล์ .zip");
        // }
      });
    },
  },
