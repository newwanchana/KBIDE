    ...
    saveFilePopUp: async function () {
      ...
      if (mode < 3) {
        let blyOption = {
          ...
          filters: [{ name: "Blockly file", extensions: ["bly", "txt"] }],
        };
        let res = dialog.showSaveDialog(null, blyOption);
        if (res) {
          let Blockly = this.$global.editor.Blockly;
          let bCode = Blockly.Xml.domToText(
            Blockly.Xml.workspaceToDom(Blockly.mainWorkspace)
          );
          let writeRes = fs.writeFileSync(
            res,
            util.b64EncodeUnicode(bCode),
            "utf8"
          );
          this.$global.ui.snackbar("Save file success");
          //--track--//
          this.$track
            .event("editor", "save", { evLabel: path.extname(res), evValue: 1 })
            .catch((err) => {
              console.log(err);
            });
        }
