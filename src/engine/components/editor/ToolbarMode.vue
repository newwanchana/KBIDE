    ...
    <v-dialog v-model="modeDialog" max-width="800px" persistent>
      <v-card>
            ...
            <v-container grid-list-md>
              <v-layout wrap>
                <template v-for="(mode, index) in modes">
                  <template
                    v-if="$global.setting.devMode === false && index + 1 === 2"
                  >
                  </template>
                  <template v-else>
                    <v-flex
                      xs12
                      :class="`md${$global.setting.devMode ? '4' : '6'}`"
                      :key="index"
                    >
                      <v-hover>
                        <v-card
                          ...
                          slot-scope="{ hover }"
                          :class="`elevation-${
                            hover ? 12 : selectingMode == index + 1 ? 8 : 2
                          }`"
                            ...
                            >
                              ...
                              >
                                ...
                                <v-flex xs2 class="text-sm-right">
                                  <i
                                    class="fa fa-check-circle fa-lg text-white"
                                  ></i>
                                </v-flex>
                              </v-layout>
                            </div>
        ...
        <v-card-actions>
          ...
          <v-btn class="btn-danger" flat @click.native="modeDialog = false">
            Close
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    window.addEventListener("keydown", (e) => {
      if (e.key === "Escape") {
        if (this.modeDialog === true) {
          console.log(
            "---------> Do something when detect escape / ToolbarMode"
          );
  ...
  methods: {
    onChangeModeDialog() {
      if (this.$global.editor.rollbackMode !== 0) {
        ...
        this.$global.$emit(
          "editor-mode-change",
          this.$global.editor.rollbackMode
        );
      ...
      if (mode >= 3) {
        ...
        if (res === "convert") {
          ...
          this.$nextTick(function () {
            //wait for element changed before fire event
