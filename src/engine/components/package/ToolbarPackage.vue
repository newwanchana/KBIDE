  ...
  <div>
    <v-tooltip bottom>
      <v-btn
        ...
        slot="activator"
        icon
        @click="packageDialog = !packageDialog"
      >
      ...
      <v-card>
        ...
        <smooth-scrollbar ref="scrollbar">
          <v-card-text>
                  ...
                  <v-list-tile :key="data.name" avatar class="list-title">
                    <v-list-tile-avatar size="60px">
                      <template v-if="data.config.image">
                        <v-img
                          contain
                          v-if="data.config.image.startsWith('http') === true"
                    ...
                    <template>
                      <v-list-tile-content class="ml-2">
                        <v-list-tile-title>
                          <strong
                            v-if="data.config.title || data.config.name"
                            >{{
                              data.config.title
                                ? data.config.title
                                : typeof data.config.name === "string"
                                ? data.config.name
                                : typeof data.config.name.en === "string"
                                ? data.config.name.en
                                : data.dirName
                            }}</strong
                          >
                          ...
                          <span class="body-1">
                            <span
                              v-if="data.config.version || data.config.author"
                            >
                              [
                              {{
                                data.config.version
                            ...
                            <span v-if="data.config.git"
                              >[
                              <a @click="openLink(data.config.git)"> git </a>
                              ]</span
                            >
                          </span>
                        </v-list-tile-title>
                        <v-list-tile-sub-title
                          v-html="data.config.description"
                        ></v-list-tile-sub-title>
                      </v-list-tile-content>
                    </template>
                    <v-list-tile-action>
            ...
            <div>
              ...
              <v-flex
                v-else-if="onlinePackageStatus === 'wait'"
                xs12
                md12
                sm12
                class="text-xs-center"
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            class="btn-primary"
            flat
            @click.native="publishNewPackage"
            >Publish your package
          </v-btn>
          <v-btn class="btn-danger" flat @click.native="packageDialog = false"
            >Close</v-btn
          >
        </v-card-actions>
      </v-card>
...
export default {
    ...
    window.addEventListener("keydown", (e) => {
      if (e.key === "Escape") {
        if (this.packageDialog === true) {
          console.log(
            "---------> Do something when detect escape / Package Manager"
          );
  ...
  methods: {
    ...
    listOnlinePackage(name = "", loadNext = false) {
      this.onlinePackageStatus = loadNext ? "OK" : "wait";
      //------------//
      let query = {
    ...
    async listLocalPackage(name = "") {
      ...
      if (name !== "") {
        ...
        this.localPackage = this.localPackage.concat(
          ...this.installedPackage.filter((obj) => {
    ...
    publishNewPackage: async function () {
      ...
      pm.publishPackage(res)
        .then((_) => {
          this.$dialog.notify.success(
            "submit your package success, please refresh again"
          );
        })
        .catch((err) => {
