<template>
  <div id="themeSetting">
    ...
    <v-container>
      <v-layout column>
        <v-flex>
          <v-subheader class="px-1 my-2"> Theme color </v-subheader>
        ...
        <div>
          <v-divider></v-divider>
          <v-flex>
            <v-subheader class="px-1 my-2"> Developer Mode </v-subheader>
            <div class="my-3">
              <v-switch
...
export default {
  ...
  watch: {
    themeColor: {
      handler(val) {
        this.lightThemeArray.find((theme) => theme === val) &&
          this.cssTextLight();
        this.darkThemeArray.find((theme) => theme === val) &&
          this.cssTextDark();
