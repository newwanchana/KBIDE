  ...
  <div>
    ...
    >
      <v-card>
        <v-card-title>
          <span class="headline"
            >Current board : {{ this.$global.board.board_info.title }}</span
          >
        ...
        <smooth-scrollbar ref="scrollbar">
          <v-card-text>
                      ...
                      <v-hover>
                        ...
                        >
                          <v-responsive @click.native="selectingBoard = data">
                            <h4
                              v-if="selectingBoard.name === data.name"
                              class="bg-success text-white pa-1 pl-2 mb-1"
                            >
                              {{ data.title }}
                            </h4>
                          ...
                          <v-card-actions>
                            <span class="ml-2"
                              ><strong>{{ data.version }}</strong></span
                            >
                            <v-spacer></v-spacer>
                            <v-btn
                              flat
            ...
            <v-divider></v-divider>
            <h3 class="ml-2 mt-2">Online available</h3>
                  ...
                  >
                    <template v-if="data.status !== 'INSTALLED'">
                      <v-card>
                        <v-responsive>
                          <h4
                            class="white--text pa-1 pl-2 primary darken-1 mb-1"
                          >
                            {{ data.title }}
                          </h4>
                          <v-img
                            contain
                            v-if="data.image.startsWith('http') === true"
                            class="board-image"
                            :src="data.image"
                          />
                          <v-img
                        ...
                        <v-card-text>
                          ...
                          >
                            <v-icon v-if="data.status === 'READY'"
                              >fa-download</v-icon
                            >
                            <v-icon v-if="data.status === 'DRAFT'"
                              >fa-pause</v-icon
                            >
                            <v-progress-circular
                          ...
                          >
                            ...
                            {{ data.status === "DRAFT" ? "(Under review) " : ""
                            }}{{ data.description }}
                          </div>
                        </v-card-text>
                        <v-divider></v-divider>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn
            v-if="$global.setting.devMode == true"
            class="btn-primary"
            flat
          ...
          <v-btn
            :class="{
              'btn-gray-400':
                this.$global.board.board_info.name === this.selectingBoard.name,
              'btn-success':
                this.$global.board.board_info.name !== this.selectingBoard.name,
            }"
            flat
          ...
          <v-btn class="btn-danger" flat @click.native="boardDialog = false"
            >Close</v-btn
          >
        </v-card-actions>
      </v-card>
...
export default {
  ...
  methods: {
    ...
    getBoardImagePath(data) {
      if (data.image && data.image.startsWith("http")) return data.image;

      const baseDir = data.dir || ""; // absolute path
      const imagePath = require("path").join(
        baseDir,
        data.image || "static/display.jpg"
      );
      return "file://" + imagePath.replace(/\\/g, "/"); // for Windows path
    },
    ...
    listOnlineBoard(name = "", loadNext = false) {
      ...
      bm.listOnlineBoard(query)
        .then((res) => {
          this.filter.currentPage = res.meta.page;
          this.filter.nextOffset = res.meta.next_offset;

          // เตรียมอัปเดต installedBoard ให้ Reactive ด้วย map
          const updatedInstalled = this.installedBoard.map((local) => {
            const online = res.boards.find(
              (obj) => obj.name.toLowerCase() === local.name.toLowerCase()
            );
            if (
              online &&
              online.version > local.version &&
              online.status !== "draft"
            ) {
              return {
                ...local,
          ...
          // อัปเดตให้ Vue detect การเปลี่ยนแปลง
          this.installedBoard = updatedInstalled;
          ...
          // filter online boards ที่ยังไม่ติดตั้ง
          const filtered = res.boards.filter((obj) => {
            const isInstalled = this.installedBoard.some(
              (local) => local.name.toLowerCase() === obj.name.toLowerCase()
            );
            if (!isInstalled) {
              obj.status =
          ...
          if (loadNext) {
            this.onlineBoards.push(...filtered);
          } else {
            this.onlineBoards = filtered;
          }
    ...
    changeBoard: async function (boardInfo) {
      ...
      if (res) {
        if (res === "confirm") {
          this.$global.board.board_info = (await bm.boards()).find(
            (obj) => obj.name === boardInfo.name
          );
