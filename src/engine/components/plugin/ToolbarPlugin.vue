  ...
  <div>
    <v-tooltip bottom>
      <v-btn
        ...
        slot="activator"
        icon
        @click="pluginDialog = !pluginDialog"
      >
    ...
    >
      <v-card>
        <v-card-title>
          ...
          >
            ...
            <v-card class="filter" max-width="350">
                    ...
                    <v-btn
                      ...
                      @click="
                        filter.categories.selected =
                          filter.categories.init_selected
                      "
                      >select all
                    </v-btn>
                    <v-btn
                      ...
                      small
                      color="primary"
                      @click="filter.categories.selected = []"
                      >select none
                    </v-btn>
                  </div>
                  <v-list-tile-action>
                    <v-item-group multiple v-model="filter.categories.selected">
                      <v-item
                        v-for="(data, index) in filter.categories.name"
                        :key="index"
                      >
                        <v-chip
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="primary" flat @click="filter.menu = false"
                  >Cancel</v-btn
                >
        ...
        <smooth-scrollbar ref="scrollbar">
          <v-card-text>
            <v-subheader> Installed </v-subheader>
            <div>
              <v-list three-line>
                  ...
                  <v-list-tile :key="data.name" avatar class="list-title">
                    <v-list-tile-avatar size="60px">
                      <template v-if="data.category.image">
                        <v-img
                          contain
                          v-if="data.category.image.startsWith('http') === true"
                    ...
                    <template>
                      <v-list-tile-content class="ml-2">
                        <v-list-tile-title>
                          <strong
                            v-if="data.category.title || data.category.name"
                            >{{
                              data.category.title
                                ? data.category.title
                                : typeof data.category.name === "string"
                                ? data.category.name
                                : typeof data.category.name.en === "string"
                          ...
                          <span class="body-1">
                            <span
                              v-if="
                                data.category.version || data.category.author
                              "
                            >
                              [
                              {{
                                data.category.version
                                  ? "v" + data.category.version + " "
                                  : ""
                              }}{{
                                data.category.author
                                  ? "by " + data.category.author
                                  : ""
                              }}
                              ]
                            </span>
                            <span v-if="data.category.git"
                              >[
                              <a @click="openLink(data.category.git)"> git </a>
                              ]</span
                            >
                          </span>
                        </v-list-tile-title>
            ...
            <v-subheader> Online available </v-subheader>
            ...
            <div>
              ...
              <v-flex
                v-else-if="onlinePluginStatus === 'wait'"
                xs12
                md12
                sm12
                class="text-xs-center"
                    ...
                    <v-list-tile-content class="ml-2">
                      <v-list-tile-title>
                        ...
                        <span class="body-1">
                          ...
                          <a v-if="data.git" @click="openLink(data.git)">
                            git
                          </a>
                          ]
                        </span>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            class="btn-primary"
            flat
            @click.native="publishNewPlugin"
            >Publish your plugin
          </v-btn>
          <v-btn class="btn-danger" flat @click.native="pluginDialog = false"
            >Close</v-btn
          >
        </v-card-actions>
      </v-card>
...
export default {
    ...
    window.addEventListener("keydown", (e) => {
      if (e.key === "Escape") {
        if (this.pluginDialog === true) {
          console.log(
            "---------> Do something when detect escape / Plugin Manager"
          );
  ...
  methods: {
    ...
    listOnlinePlugin(name = "", loadNext = false) {
      ...
      //------------//
      let selected_categories = this.filter.categories.selected.map(
        (el) => this.filter.categories.name[el]
      );
      let query = {
    ...
    listLocalPlugin(name = "") {
      ...
      this.localPlugin = [];
      this.installedPlugin = pm
        .plugins(this.$global.board.board_info)
        .map((obj) => {
          obj.status = "READY";
          return obj;
        });
      if (name !== "") {
        ...
        this.localPlugin = this.localPlugin.concat(
          ...this.installedPlugin.filter((obj) => {
    ...
    publishNewPlugin: async function () {
      ...
      pm.publishPlugin(res)
        .then((_) => {
          this.$dialog.notify.success(
            "submit your plugin success, please refresh again"
          );
        })
        .catch((err) => {
