          ...
          <v-card-text style="padding: 0">
            <v-layout row wrap class="form-group">
              ...
              <v-flex
                ...
                sm6
                class="pt-3"
                v-if="plugin_info.board.length === 0"
              >
                <label>Platforms</label>
                <v-layout row wrap>
                  <v-flex sm3 md3 xs3>
                    <v-checkbox
                      v-model="plugin_info.platform"
                      label="esp-idf"
                      value="esp-idf"
                    ></v-checkbox>
                  </v-flex>
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn class="btn" color="primary" flat @click.native="step = 1"
                  >Prev</v-btn
                >
                <v-btn
                  class="btn-danger"
                  flat
                  @click.native="pluginDialog = false"
                  >Close</v-btn
                >
              </v-card-actions>
            </v-layout>
...
export default {
  ...
  data() {
    return {
      ...
      text64Rules: [
        (v) => !!v || "required",
        (v) => (v && v.length <= 64) || "text must be less than 64 characters",
      ],
      descriptionRules: [
        (v) => !!v || "required",
        (v) =>
      ...
      gitUrlRules: [
        ...
        (v) =>
          /^http(s)?:\/\/(www\.)?github\.com\/[\.\_\-0-9A-Za-z]+\/[\.\_\-0-9A-Za-z]+\/$/g.test(
            v
          ) || "Git must be valid (start with http(s)? and end with '/'",
      ],
    };
  },
  created() {
  ...
  methods: {
    parseConfig: function () {
      ...
      let json;
      request_promise(
        this.plugin_info.git +
          "raw/master/library.json?random=" +
          util.randomString()
