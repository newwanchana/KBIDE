      ...
      <v-flex xs4>
        <v-card>
          <v-card-text class="px-0 text-xs-left pl-3 pt-0 pb-0 ma-0">
            ...
            <span class="caption">
              Board : {{ $global.board.board_info.title }} ({{
                $global.board.board_info.name
              }})
            </span>
          </v-card-text>
        </v-card>
      ...
      <v-flex xs2>
        <v-card>
          <v-card-text class="px-0 text-xs-center pa-0 ma-0">
            <span class="caption"> Mode : {{ $global.editor.mode }} </span>
          </v-card-text>
        </v-card>
      </v-flex>
      <v-flex xs6>
        <v-card>
          <v-card-text class="px-0 text-xs-right pr-3 pt-0 pb-0 ma-0">
            ...
            <span class="caption"> &nbsp;&nbsp;{{ editorStatus }} </span>
          </v-card-text>
        </v-card>
      </v-flex>
    </v-layout>
  ...
  methods: {
    mountCm: function () {
      if (this.$global.editor.Editor) {
        ...
        this.$global.editor.Editor.onDidChangeCursorPosition(
