<template>
  <v-list>
    <v-list-group v-for="(item, index) in nodes" :key="index">
          ...
          <v-list-tile-content>
            <v-list-tile-title style="height: 34px">
              <div style="display: flex">
                <img
                  src="/static/icons/SVG/201556.svg"
                  alt=""
      ...
      >
        <v-expansion-panel popout>
          ...
          >
                ...
                <v-layout row wrap align-center>
                  ...
                  <v-flex xs12 sm6>
                    <v-btn
                      v-if="
                        example.examples &&
                        example.examples.find((obj) => obj.endsWith('.bly'))
                      "
                      small
                      color="primary"
                      @click.stop=""
                    ...
                    <v-btn
                      v-if="
                        example.examples.find((obj) => obj.endsWith('.ino'))
                      "
                      small
                      color="primary"
                      @click.stop=""
            ...
            <v-card>
              <v-card-text
                v-if="example.examples.find((obj) => obj.endsWith('.md'))"
              >
                <div class="vue-markdown">
                  <vue-markdown v-on:rendered="editTag"
                    >{{ getMarkdown(example) }}
                  </vue-markdown>
                </div>
              </v-card-text>
          ...
          for (let i = 0; i < aTags.length; i++) {
            ...
            if (originalHref.indexOf("#") < 0) {
              aTags[i].setAttribute(
                "onclick",
                "require('electron').shell.openExternal('" + originalHref + "')"
              );
              aTags[i].href = "#";
            }
          }
