<template>
  <v-toolbar color="primary" fixed dark app height="64px">
    ...
    <v-tooltip bottom>
      <template v-slot:activator="{ on }">
        <img
          v-on="on"
          src="/static/logo/KBIDE.png"
          height="45"
          alt="Problem? report me at fb.com/comdet"
          class="mr-3"
          style="
            border-radius: 8px;
            background-color: #ffffffa8; //margin-top: 4px;
          "
        />
      </template>
      <span>KBIDE by MakerAsia | PRINCEBOT CO., LTD.</span>
    </v-tooltip>
    <v-tooltip bottom>
      <template v-slot:activator="{ on }">
        <img
          v-on="on"
          src="/static/logo/kidcom_128.png"
          height="45"
          alt="KidBright Community"
          class="mr-2"
          style="
            border-radius: 8px;
            background-color: rgba(255, 255, 255, 0.8); //margin-top: 4px;
          "
        />
      </template>
...
export default {
  ...
  methods: {
    ...
    processToolbar: async function (t = this) {
      t.toolbarComp = [];
      t.actionbarComp = [];
      let boardActionBar = await bm.listActionbar(
        Vue.prototype.$global.board.board
      );
      let boardToolbar = await bm.listToolbar(
        Vue.prototype.$global.board.board
      );
      for (let packageName in Vue.prototype.$global.board.package) {
        t.actionbarComp.push({
          info: Vue.prototype.$global.board.package[packageName],
          name: packageName,
          comp: boardActionBar[packageName],
        });
        t.toolbarComp.push({
          info: Vue.prototype.$global.board.package[packageName],
          name: packageName,
          comp: boardToolbar[packageName],
        });
      }
    },
    processAppToolbar: async function (t = this) {
      ...
      for (let packageName in Vue.prototype.$global.packages) {
        t1.push({
          info: Vue.prototype.$global.packages[packageName],
          name: packageName,
          comp: appActionbar[packageName],
        });
        t2.push({
          info: Vue.prototype.$global.board.package[packageName],
          name: packageName,
          comp: appToolbar[packageName],
        });
      }
